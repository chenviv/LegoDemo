...
struct SensorData {
  float temp;       // Temperature (°C) - 4 bytes
...
void setup() {
  ...
  // **ADD THIS: Print the calculated offsets to verify**
  ...
  Serial.println("Waiting for client (28-byte binary packets)...");
}

void loop() {
  mpu.update();

  if (deviceConnected && (millis() - timer > 100)) {
    SensorData data;
    ...
    // Debug: Print values
    Serial.println("\n--- Raw Sensor Check (device should be FLAT and STILL) ---");
    Serial.printf("Accel: X=%.3f Y=%.3f Z=%.3f (mag=%.3f)\n", 
